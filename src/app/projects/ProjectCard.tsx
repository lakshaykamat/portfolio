...
const ProjectCard = (props: Props) => {
  const TECH_STACK = props.techStack.map((item, index) => {
    return (
      <span
        key={index}
        className="inline-block px-2 py-1 mb-2 mr-2 text-sm text-gray-400 bg-gray-700 rounded"
      >
        {item}
      </span>
    );
  ...
  return (
    <motion.div
      ...
      className="relative flex flex-col overflow-hidden rounded shadow-lg bg-slate-800"
    >
      ...
      <div className="px-6 py-4">
        <a href={props.githubLink.toString()} className="btn-indigo">
          Source Code
        </a>
      </div>
    </motion.div>
